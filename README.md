...
- [コサイン類似度データの中の外れ値をもつファイルを見つける方法](#コサイン類似度データの中の外れ値をもつファイルを見つける方法)
  ...
  - [補足：コサイン類似度と計算](#補足コサイン類似度と計算)
    - [コサイン類似度の計算その1: ベクトルの内積の計算](#コサイン類似度の計算その1-ベクトルの内積の計算)
    - [コサイン類似度の計算その2: 各ベクトルのノルムを計算](#コサイン類似度の計算その2-各ベクトルのノルムを計算)
    - [コサイン類似度の計算その3: コサイン類似度を求める](#コサイン類似度の計算その3-コサイン類似度を求める)
...
世の中は大規模言語モデルや動画生成AIが話題の中心になっていますが、現実的には画像分類、特にオープンセット分類[^0]の需要はますます高まっています。
...
各手法にはそれぞれ利点と欠点があり、適切な方法を選ぶためにはデータの特性と検出の目的を理解することが重要です。例えば、単純な閾値設定は簡単である反面、分布の形状によっては効果が薄く、t-SNEは強力ですが、計算コストが高いため用途を選びます。

最も重要なのは、外れ値検出の目的に応じて適切な手法を選択し、視覚化を活用してデータをより深く理解することです。
