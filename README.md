...
2つのベクトル $a$ と $b$ のコサイン類似度は次の式で定義されます。
...
### その1: ベクトルの内積の計算
2つのベクトル $a$ と $b$ の内積は、2つのベクトルの対応する要素を掛け合わせたものを全て足し合わせて求めます。
...
### その2: 各ベクトルのノルムを計算
次にベクトル $a$ と $b$ のノルム（大きさ）を計算します。各ノルムは、各要素を2乗して合計した数の平方根をとることで得られます。
